...
function initializeCertificatesChart() {
  ...
  charts.certificates = new Chart(ctx, {
    type: "line",
    data: {
      labels: certificateData.map((c) => c.month) || [
        "Jan",
        "Feb",
        "Mar",
        "Apr",
      ...
      datasets: [
        {
          ...
          data: certificateData.map((c) => c.certificatesIssued) || [
            10, 15, 12, 20, 18, 25,
          ],
...
function initializePointsChart() {
  ...
  charts.points = new Chart(ctx, {
    type: "bar",
    data: {
      labels: pointsData.map((p) => p.month) || [
        "Jan",
        "Feb",
        "Mar",
        "Apr",
      ...
      datasets: [
        {
          label: "Points Earned",
          data: pointsData.map((p) => p.totalPointsEarned) || [
            1200, 1500, 1300, 1800, 1600, 2000,
          ],
      ...
      datasets: [
        {
          ...
          data: chatbotData.map((c) => c.conversationCount) || [
            5, 8, 12, 7, 15, 10, 18,
          ],
      ...
      datasets: [
        {
          ...
          data: revenueData.map((r) => r.revenue) || [
            5000, 7500, 6200, 8900, 7800, 12000,
          ],
          borderColor: "#8b5cf6",
          backgroundColor: function (context) {
            const chart = context.chart;
  ...
  switch (chartType) {
    ...
    case "certificates":
      if (data.certificateData) {
        ...
        chart.data.datasets[0].data = data.certificateData.map(
          (c) => c.certificatesIssued
        );
      }
      break;
    ...
    case "points":
      if (data.monthlyData) {
        ...
        chart.data.datasets[0].data = data.monthlyData.map(
          (m) => m.totalPointsEarned
        );
      }
      break;
    ...
    case "chatbot":
      if (data.dailyUsage) {
        ...
        chart.data.datasets[0].data = data.dailyUsage.map(
          (d) => d.conversationCount
        );
      }
      break;
...
function showActivityTab(tabName) {
  ...
  const activeTab = document.querySelector(
    `[onclick="showActivityTab('${tabName}')"]`
  );
  if (activeTab) activeTab.classList.add("active");

  // Show/hide content
  document
    .querySelectorAll(".activity-tab-content")
