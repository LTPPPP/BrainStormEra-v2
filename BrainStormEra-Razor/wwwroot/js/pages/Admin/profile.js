...
function showToast(message, type = "info") {
  ...
  const toastHtml = `
        <div id="${toastId}" class="toast align-items-center text-white bg-${getBootstrapColorClass(
    type
...
function addLoadingStates() {
  ...
  const actionButtons = document.querySelectorAll(
    ".profile-actions .btn, .quick-action-item"
  );
...
async function refreshQRCode() {
  try {
    ...
    const description =
      document.getElementById("qrDescription").value || "Payment";

    const bankName = document.getElementById("qrBankName").textContent;
    const accountHolder =
      document.getElementById("qrAccountHolder").textContent;
    const accountNumber =
      document.getElementById("qrAccountNumber").textContent;
  ...
  try {
    ...
    if (result.success) {
      ...
      const avatarImages = document.querySelectorAll(
        "#profileAvatar, #avatarPreview, #previewAvatar"
      );
