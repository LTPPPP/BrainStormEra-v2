...
class QuizTakeManager {
    ...
    checkTimeWarnings() {
        ...
        if (this.timeRemaining <= 1) { // 1 minute warning
            $timer.addClass('danger');
            $warning.show();
        } else if (this.timeRemaining <= 5) { // 5 minute warning
    ...
    hasAnswers() {
        return $('input[type="radio"]:checked').length > 0 || 
               $('textarea').filter(function() { return $(this).val().trim() !== ''; }).length > 0;
    }
    ...
    saveDraft() {
        ...
        console.log('Save draft functionality would go here');
    ...
    highlightUnansweredQuestions() {
        ...
        $('.question-card').each(function() {
            const $card = $(this);
            const hasRadioAnswer = $card.find('input[type="radio"]:checked').length > 0;
