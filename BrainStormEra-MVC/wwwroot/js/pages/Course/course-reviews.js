...
class CourseReviews {
  ...
  initializeElements() {
    ...
    this.eligibilityMessage = document.getElementById(
  ...
  attachEventListeners() {
    ...
    if (this.cancelReviewBtn) {
      this.cancelReviewBtn.addEventListener("click", () =>
        this.hideReviewForm()
      );
    }
    ...
    if (this.createReviewForm) {
      this.createReviewForm.addEventListener("submit", (e) =>
        this.handleSubmitReview(e)
      );
    }
  ...
  updateRatingText() {
    ...
    if (this.selectedRating > 0) {
      this.ratingText.textContent = `${this.selectedRating} star${
        this.selectedRating > 1 ? "s" : ""
  ...
  async checkReviewEligibility() {
    try {
      const response = await fetch(
        `/Course/CheckReviewEligibility?courseId=${this.courseId}`
      );
      ...
      this.showEligibilityMessage(
        "Unable to check review eligibility. Please try again.",
        "error"
      );
    }
  }
  ...
  populateFormForEdit(reviewData) {
    ...
    const starInput = this.starRatingInput.querySelector(
      `input[value="${reviewData.starRating}"]`
    );
    if (starInput) {
  ...
  updateFormForEdit() {
    ...
    if (this.submitReviewBtn) {
      this.submitReviewBtn.innerHTML =
  ...
  setSubmitButtonLoading(isLoading) {
    if (isLoading) {
      ...
      const loadingText = this.editingReviewId
        ? "Updating..."
        : "Submitting...";
      this.submitReviewBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${loadingText}`;
    } else {
      this.submitReviewBtn.disabled = false;
      const buttonText = this.editingReviewId
        ? "Update Review"
        : "Submit Review";
      const iconClass = this.editingReviewId
        ? "fas fa-save"
        : "fas fa-paper-plane";
  ...
  async refreshReviews() {
    try {
      const response = await fetch(
        `/Course/GetCourseReviews?courseId=${this.courseId}&page=1&pageSize=10`
      );
      const result = await response.json();
      ...
      // Handle refresh error silently as this is a background operation
    }
  }
  ...
  generateReviewHtml(review) {
    ...
    const actionButtons = isCurrentUserReview
      ? `<div class="review-actions-buttons">
                <button class="btn btn-sm btn-outline-primary edit-review-btn" onclick="courseReviews.editReview('${
                  review.reviewId
                }', ${review.starRating}, '${(
          review.reviewComment || ""
                ...
                <button class="btn btn-sm btn-outline-danger delete-review-btn" onclick="courseReviews.deleteReview('${
                  review.reviewId
                }')">
  ...
  updateRatingSummary(averageRating, totalReviews) {
    ...
    if (ratingCount) {
      ratingCount.textContent = `(${totalReviews} review${
        totalReviews !== 1 ? "s" : ""
      })`;
    }
  }
