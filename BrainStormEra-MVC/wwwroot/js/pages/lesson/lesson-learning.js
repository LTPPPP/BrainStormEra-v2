...
function markAsComplete() {
  // OLD markAsComplete() called - this should NOT be called!
  if (window.showToast) {
    window.showToast(
      "info",
      "OLD function called - Mark as complete functionality will be implemented"
    );
  } else {
    alert(
      "OLD function called - Mark as complete functionality will be implemented"
...
function markAsCompleteNew() {
  ...
  const completeBtn = document.querySelector(
    'button[onclick="markAsCompleteNew()"]'
  );
  if (completeBtn) {
    ...
    .then((data) => {
      if (data.success) {
        if (window.showToast) {
          window.showToast(
            "success",
            data.message || "Lesson completed successfully!"
          );
        } else {
...
function getLessonId() {
  ...
  const hiddenInput = document.querySelector(
    'input[name="lessonId"], input[name="LessonId"]'
  );
  if (hiddenInput) {
...
function getCourseId() {
  ...
  const hiddenCourseId = document.querySelector(
    'input[name="courseId"], input[name="CourseId"]'
  );
  if (hiddenCourseId) {
  ...
  // Could not find course ID from any source
  return null;
}
...
function updateSidebar(chapters) {
  if (!chapters || !Array.isArray(chapters)) {
    // Invalid chapters data for sidebar update
    return;
  }
        ...
        if (lessonElement) {
          ...
          if (lesson.progressPercentage !== undefined) {
            const progressElement =
              lessonElement.querySelector(".progress-bar");
            if (progressElement) {
...
function updateCourseProgress(progressPercentage) {
  ...
  const progressPercentageElements = document.querySelectorAll(
    ".progress-percentage"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  const lessonType = document
    .querySelector("[data-lesson-type]")
    ?.getAttribute("data-lesson-type");

  if (lessonType && lessonType.toLowerCase() === "video") {
    // TODO: Implement video progress tracking
    // Video lesson detected - progress tracking enabled
  }
});
